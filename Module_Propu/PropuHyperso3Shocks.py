...
def Shock(Mach,P,Rho,T,Beta=math.pi/2,Teta=0):
    ...
    Mn2 = (1+((Gamma-1)/2)*Mn1**2)/(Gamma*(Mn1**2)-((Gamma-1)/2))
...
def intake3Shock(Mach,Altitude):
    ...
    Beta1 = Angle1(Mach)
    Beta2 = Angle2(Mach)
    # Beta1=math.radians(25)
    # Beta2=math.radians(25)
...
def GraphRamjet(IntakeDiameter,D4,X4,D5,X5):
    Upper = np.array([0,0])
...
def RamJet(Mach, Altitude, IntakeDiameter,TCombustion):
    M4,P4,Rho4,T4,Pinf,Vinf,Beta1,Beta2,Teta = intake3Shock(Mach,Altitude)
    ...
    GraphRamjet(IntakeDiameter,D4,X4,D5,X5)
...
if __name__ == "__main__":
    ...
    IntakeDiameter = 1
    TCombustion = 3400
    RamJet(Mach, Altitude, IntakeDiameter,TCombustion)
